...
4. **The golden set is sampled with free keyword labels and hand-labeled row by row.** The original plan had the LLM pre-label the golden set, leaving most gold labels equal to the classifier's own output. That grades the classifier against itself. Instead, keyword rules (no API cost) pick a stratified sample of 200, and every row's gold intent and escalation label was assigned by reading the message against the written rubric, with Claude as a labeling assistant. Gold intent differs from the keyword prefill on 101 of 200 rows. *Cost if wrong:* a single labeler, not an independent panel (see the report's "misleading" section).
