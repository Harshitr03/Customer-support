...
11. **Human-vs-judge agreement is scored blind, on all 3 systems per spot-check message, not a rotation.** The human scores every (message, system) reply pair from the 40 spot-check messages (120 pairs total, ~40 per system), without seeing the judge's score or which system wrote the reply, so the judge can't anchor the human. I report agreement as binned Cohen's kappa, quadratic-weighted kappa, Spearman correlation, and exact and within-one agreement, with a bootstrap confidence interval. Result: binned kappa 0.155 [-0.001, 0.305], weighted kappa 0.349, Spearman 0.382, exact 0.425, within-one 0.767 -- and per system, grounded 0.388, trivial 0.181, nearest -0.017 (essentially chance). An earlier round scored only one system per message in rotation (40 pairs, ~13 per system) and reported a wider, more optimistic kappa of 0.342 [-0.074, 0.692] -- the full 120 halved the CI width but *lowered* the point estimate, the expected signature of a small-sample overestimate getting corrected, not a real regression. The finding that survived: the judge is a real, if weaker-than-first-measured, signal on grounded replies, and cannot distinguish a fluent-but-wrong-customer `nearest` reply (e.g. "Hey Dylan" to a customer named Diana) from a good one. *Cost if wrong:* the reply-quality headline (§4 of the report) rests on a judge whose reliability is now measured, not assumed, and it's weaker than the first small sample suggested -- disclosed rather than left at the more flattering number.
