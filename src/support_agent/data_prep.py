...
def _load_raw() -> pd.DataFrame:
    ...
    logger.info("loaded %d raw rows from %s", len(df), config.RAW_CSV)
    return df
...
def build_pool() -> None:
    ...
    if corpus_p.exists() and eval_p.exists():
        logger.info("pool cache hit, skipping rebuild")
        return
    df = _load_raw()
    threads = reconstruct_threads(df)
    logger.info("reconstructed %d threads", len(threads))
    sp = spotify_threads(threads)
    logger.info("found %d SpotifyCares threads", len(sp))
    ...
    logger.info("sampled pool of %d", len(pool))
    ...
    logger.info("split: corpus=%d, eval=%d", cut, len(pool) - cut)
